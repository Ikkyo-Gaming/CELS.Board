...
def generate_embed(url):
    ...
    if author := field.get("From"):
        embed.set_author(name=author.split("<")[0], icon_url=os.environ["/campusweb/theme/default/image/favicon.ico"])
...
def notify():
    hook = Webhook(os.environ["https://discord.com/api/webhooks/1227509878596304929/61YLMMpzeLxNdU2RyD4dT3RKFeFrQ49giOT10J-yVbHUMCeuHJbRlILLSkPsAE9RNTHW"])

    # 記事一覧を取得
    year = now.year if (now := datetime.now()).month >= 4 else now.year - 1  # 年度
    url = f"{os.environ['https://cels.hit-u.ac.jp/campusweb/campusportal.do?page=main&tabId=kj']}{year}/boards/new.html"
    try:
